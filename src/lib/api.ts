...
/**
 * DEBOUNCE MIDDLEWARE
 * Prevents rapid-fire requests by waiting for a quiet period.
 * This is applied to all backend requests to ensure stability.
 */
const requestCache = new Map<string, { timeout: ReturnType<typeof setTimeout>; resolve: any; reject: any }>();

const debouncedRequest = <T>(key: string, fn: () => Promise<T>, wait = 300): Promise<T> => {
  return new Promise((resolve, reject) => {
    ...
    requestCache.set(key, { timeout, resolve, reject });
  });
};
...
export const addPunch = async (username: string, dateStr: string, timeStr: string, isManual: boolean = false) => {
  ...
  return debouncedRequest(`addPunch-${username}-${dateStr}-${timeStr}-${isManual}`, async () => {
...
export const fetchDashboardByDate = async (username: string, dateStr: string) => {
  return debouncedRequest(`fetchDashboardByDate-${username}-${dateStr}`, async () => {
...
export const fetchMonthlyHistory = async (username: string, monthYearStr: string) => {
  return debouncedRequest(`fetchMonthlyHistory-${username}-${monthYearStr}`, async () => {
...
export const logLeaveHours = async (username: string, dateStr: string, hours: number) => {
  return debouncedRequest(`logLeaveHours-${username}-${dateStr}`, async () => {
    const { data, error } = await supabase
      .from('user_pto')
      .upsert({ 
        username: username, 
...
export const fetchUserPto = async (username: string) => {
  return debouncedRequest(`fetchUserPto-${username}`, async () => {
    const { data, error } = await supabase
      .from('user_pto')
      .select('*')
...
export const deletePtoRecord = async (username: string, pto_date: string) => {
  return debouncedRequest(`deletePtoRecord-${username}-${pto_date}`, async () => {
...
export const updateShiftSettings = async (username: string, scheduleKey: '0900' | '0930' | '1000' | '1300') => {
  return debouncedRequest(`updateShiftSettings-${username}`, async () => {
    const schedules = {
...
export const fetchUserSettings = async (username: string) => {
  return debouncedRequest(`fetchUserSettings-${username}`, async () => {
    const { data, error } = await supabase
      .from('user_settings')
      .select('*')
      .eq('username', username)
...
export const fetchRawPunchesByDate = async (username: string, dateStr: string) => {
  return debouncedRequest(`fetchRawPunchesByDate-${username}-${dateStr}`, async () => {
...
export const deleteSpecificPunch = async (id: number) => {
  return debouncedRequest(`deleteSpecificPunch-${id}`, async () => {
    const { error } = await supabase
      .from('raw_punches')
      .delete()
      .eq('id', id);
